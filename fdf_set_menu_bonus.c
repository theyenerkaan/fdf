...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   fdf_set_menu_bonus.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: yenyilma <yyenerkaan1@student.42.fr>       +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/05 22:15:06 by yenyilma          #+#    #+#             */
/*   Updated: 2025/01/26 16:48:56 by yenyilma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_centered_bold_text(t_fdf *fdf, t_menu_params *params,
	char *text, int color)
{
	int	centered_x;

	centered_x = params->x + params->padding
		+ ((params->width - params->padding
				- (ft_strlen(text) * 10)) / 2);
...
static void	set_menu_controls(t_fdf *fdf, t_menu_params *params)
{
	params->spacing = 37;
	params->padding = 10;
	draw_centered_bold_text(fdf, params, "=== Controls ===", 0xFFFFFF);
	params->spacing = 25;
...
{
	params->spacing = 37;
	draw_centered_bold_text(fdf, params, "Projection:", 0xFFFFFF);
	params->spacing = 25;
	draw_centered_bold_text(fdf, params,
		"1 - Isometric ( ~30° )", params->color);
	draw_centered_bold_text(fdf, params,
		"2 - Dimetric ( ~37° )", params->color);
	...
	params->spacing = 25;
	draw_centered_bold_text(fdf, params,
		"Quit: Q or ESC", params->color);
	params->spacing = 57;
	draw_centered_bold_text(fdf, params,
		"Start: Any of the Zoom Keys", params->color);
}
